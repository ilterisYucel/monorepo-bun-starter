> **SUPERSEDED (2026-08-19):** Bu doküman geçersizdir — yerine geçen mimari ve uygulama planı: [EDITOR-MIMARISI.md](../architecture/EDITOR-MIMARISI.md). Önerilen yeni platform paketi (`config-engine`) iptal edildi: editör mantığı `apps/editor` içinde yaşar, validasyon `shared-types` şemalarını kullanır. Tarihsel kayıt olarak korunur.
