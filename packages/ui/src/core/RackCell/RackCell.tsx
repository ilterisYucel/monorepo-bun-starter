...
export const RackCell: React.FC<
  RackCellProps & { frameCount: number; timestampRef: { current: number } }
> = ({ rack, x, y, config, flowDirection, frameCount, timestampRef, onClick }) => {
  const drawBody = useCallback(
    (g: GraphicsType) => {
      g.clear();
      drawRackBody(g, config);
    },
    [config],
  );
  ...
  const drawTerminals = useCallback(
    (g: GraphicsType) => {
      g.clear();
      drawRackTerminals(g, rack, config);
    },
    [rack, config],
  );
  ...
  const drawFill = useCallback(
    (g: GraphicsType) => {
      g.clear();
      drawRackFill(g, rack, config, flowDirection);
    },
    [rack, config, flowDirection, frameCount],
  );

  const drawGlow = useCallback(
    (g: GraphicsType) => {
      g.clear();
      drawRackGlow(g, rack, config, flowDirection, timestampRef);
    },
    [rack, config, flowDirection, frameCount],
  );
