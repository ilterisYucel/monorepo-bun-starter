...
export function drawRackGlow(
  ...
  const { rackWidth: w, rackHeight: h, step } = cfg;
  const r = w * 0.42;
  const pulse = 1 + Math.sin(timestampRef.current * 0.004) * 0.4;
  const color = flowDirection === "Charge" ? 0x10b981 : 0xf59e0b;
  const alpha = 0.15 + Math.sin(timestampRef.current * 0.004) * 0.08;
...
export function drawRackTerminals(
  ...
  const borders = [
    0x3d3d5e,       // row 0: rack ID
    statusBorder,   // row 1: status
    chargeBorder,   // row 2: charge
    0x3b82f6,       // row 3: SoC
    0x3b82f6,       // row 4: voltage
    0xf59e0b,       // row 5: current
  ];
