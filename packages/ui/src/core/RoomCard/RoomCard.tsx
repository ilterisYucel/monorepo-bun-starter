...
export const RoomCard: React.FC<RoomCardProps> = ({
  ...
  const drawTempBar = useCallback(
    ...
    [roomPos, room.temp, config, frameCount],
  );
  ...
  return (
    <pixiContainer>
      ...
      <pixiText
        text={`O${roomPos.index + 1}`}
        x={roomPos.x + roomPos.width / 2}
        y={roomPos.y + config.step * 0.7}
        anchor={0.5}
        style={{ fontSize: fs, fill: 0xe5e7eb, fontFamily: "monospace", fontWeight: "bold" }}
      />
      <pixiText
        text={`${room.temp.toFixed(1)}°C`}
        x={roomPos.x + roomPos.width / 2}
        y={roomPos.y + config.step * 1.5}
        anchor={0.5}
