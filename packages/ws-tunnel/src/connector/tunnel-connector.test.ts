...
import { TunnelConnector } from "./tunnel-connector";
import type {
  ISocketClient,
...
import { TUNNEL_PROTOCOL_VERSION } from "../protocol";

import type { TunnelTelemetryPoint } from "../types";


/**
 * T2.1b — TunnelConnector sözleşmesi (tasarım §4.3 + §6 diyagramı):
 *
 ...
 * - open → register {peerId, peerType, protocolVersion:2} gönderilir; registerTimeoutMs
...
function makeConfig(overrides: Partial<TunnelConnectorTestConfig> = {}) {
  return {
...
type TunnelConnectorTestConfig = ReturnType<typeof makeConfig>;
...
describe("TunnelConnector (T2.1b)", () => {
  ...
  describe("start() / bağlanma", () => {
    ...
    it("open sonrası register mesajı gönderir (peerId + peerType + protocolVersion)", () => {
      void connector.start();
  ...
  describe("stop() yarışları", () => {
    ...
    it("stop sonrası rejected yeniden bağlanmaz (tek URL)", () => {
      const single = new TunnelConnector(
  ...
  describe("logger'sız çalışma", () => {
    it("log çağrıları atlanır — çökme yok", () => {
      const bare = new TunnelConnector(
        makeConfig(),
        factory,
        { snapshot },
        new ReconnectDelay({ baseMs: 1000, maxMs: 60000, jitterSpanMs: 0, jitter: () => 0 }),
  ...
  describe("constructor doğrulaması", () => {
    ...
    it("boş peerId reddedilir", () => {
      expect(() => mk({ peerId: "" })).toThrow();
    });
