...
import { ClientSessionStore } from "./client-session-store";
import { JoseSignerForTests } from "./__test-helpers__/jose-signer";
import type { ClientSessionUser } from "./client-session-store";

/**
 * T3.3 container tarafı — ClientSessionStore sözleşmesi (tasarım §5.4/§5.7):
 *
...
const user: ClientSessionUser = { id: "u-1", username: "operator", role: "guest" };
...
function makeStore(overrides: {
  ...
  const store = new ClientSessionStore(adapter, overrides);
  return store;
}

describe("ClientSessionStore (T3.3)", () => {
  beforeEach(() => {
