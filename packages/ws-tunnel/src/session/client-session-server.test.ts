...
import { ClientSessionServer } from "./client-session-server";
...
/**
 * T3.3 container tarafı — ClientSessionServer sözleşmesi (tasarım §5.7):
...
describe("ClientSessionServer (T3.3)", () => {
  it("open-session → open-session-ack (token + expiresInSec)", async () => {
    ...
    const server = new ClientSessionServer(fake.connector, store, undefined);
    server.start();
